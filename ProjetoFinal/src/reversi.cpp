...
void Reversi::checar_jogada_multidirecional(int i, int linha, int coluna, char jogador) {
	...
	else for(int j = i+1; j < JogadasValidas.size(); j++) {
...
bool Reversi::game_over()
{
	...
	if (termino==2||num_pecas_X==0||num_pecas_O==0)
	{
		...
		std::cout <<std::endl << "Numero de pecas do Jogador 1: " << num_pecas_X << std::endl;
		std::cout << std::endl << "Numero de pecas do Jogador 2: " << num_pecas_O << std::endl;
		return (false);
	}
